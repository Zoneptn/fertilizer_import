...
def load_data():
    ...
    regis_df.columns = regis_df.columns.str.strip()
...
# ==========================================================
# Companies Selling Each Fertilizer Formula (Active Only)
# ==========================================================

st.divider()
st.header("🏢 Companies Selling Each Fertilizer Formula")


# Keep only active registrations
regis_df["Expiry_Date"] = pd.to_datetime(regis_df["Expiry_Date"], errors="coerce")
today = pd.Timestamp.today().normalize()

active_df = regis_df[regis_df["Expiry_Date"] >= today].copy()

# Count active companies by formula
formula_company = (
    active_df.groupby("Formula")["Company_Name"]
             .nunique()
...
formula = st.selectbox(
    "Select Fertilizer Formula",
    sorted(active_df["Formula"].dropna().unique())
)
...
summary = (
    active_df.groupby("Formula")
             .agg(
                 Number_of_Companies=("Company_Name", "nunique"),
                 Companies=("Company_Name", lambda x: ", ".join(sorted(set(x))))
             )
             .reset_index()
             .sort_values("Number_of_Companies", ascending=False)
)
